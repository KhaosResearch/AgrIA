...
export class ChatService {
  ...
  public loadParcelDataToChat(request: FormData): Observable<IChatParcelResponse> {
    return this.http
      .post<{ response: IChatParcelResponse }>(`${this.apiUrl}/load-parcel-data-to-chat`, request)
      .pipe(map(res => res.response));
  }

  public loadInputSuggestion(request: FormData): Observable<string> {
    return this.http.post<{ response: string }>(`${this.apiUrl}/get-input-suggestion`, request).pipe(map(res => res.response));
  }
}
