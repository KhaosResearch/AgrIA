...
# Assuming step 3 report node is imported here later

def simulate_graph_run(initial_state: AgrIAState, client, model_name: str) -> AgrIAState:
    print(f"\n--- Starting AgrIA Graph Routing ---")
    print(f"Active Language Context: {initial_state['lang']}")
    
    # 1. Evaluate the routing logic deterministically
    next_node = deterministic_router(initial_state)
    print(f"Decision: Router directed transaction to -> [{next_node}]")
    ...
    if next_node == "basic_chat":
        ...
        print("Routing to report generator (Step 3)...")
        updated_state = generate_report_node(initial_state, client, model_name)
        
    print(f"Transaction Complete. Tokens saved by bypassing monolithic loading.")
...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------
    print("\n[TEST A] Initiating Greeting Session...")
    state_a: AgrIAState = {
    ...
    print(f"-> Final Node Output: {result_a['messages'][-1]}\n")

    # ----------------------------------------------------------------
    # TEST CASE B: Out of Scope Prompt (Routes to fallback_rejection)
    # ----------------------------------------------------------------
    print("\n[TEST B] Initiating Out-of-Scope Prompt...")
    state_b: AgrIAState = {
    ...
    print(f"-> Final Node Output: {result_b['messages'][-1]}\n")

    # ----------------------------------------------------------------
    # TEST CASE C: Core Analysis Trigger (Routes to report_generator)
    # ----------------------------------------------------------------
    print("\n[TEST C] Initiating Report Generation Flow...")
    ...
    state_c: AgrIAState = {
        ...
        "messages": [HumanMessage(content="###DESCRIBE_SHORT_IMAGE### Please review this parcel data.")],
        "lang": "es",
