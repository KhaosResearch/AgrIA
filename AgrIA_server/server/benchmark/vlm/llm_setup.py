...
def generate_system_instructions(prompt_json_path: str = BM_PROMPT_LIST_FILE):
    ...
    classification_data = load_prompt_from_json(
        PROMPT_LIST_FILE, 'classification')
    examples_data = load_prompt_from_json(
        prompt_json_path, 'examples', True, base_path=BM_DIR)
    ...
    system_instructions = role_prompt + EXCLUSIVITY_RULE + \
        CALCULATIONS_RULE + classification_instruction + examples_instructions
