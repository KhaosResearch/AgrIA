...
def setup_ndates(
    n_dates: int, to_date: datetime = datetime.today(), delta_days: int = 4 * 365
):
    # Get all parcel descriptions
    from_date = to_date - timedelta(days=delta_days)
    dates = n_random_dates_between(
        from_date.strftime("%Y-%m-%d"), to_date.strftime("%Y-%m-%d"), n_dates
    )
...
def get_parcel_data_and_description(
    cadastral_ref: str, image_date: str, lang: str = LANG
):
    if not lang:
        lang = LANG
    ...
    if isinstance(
        parcel_metadata, dict
    ) and "TOTAL ELIGIBLE SURFACE (ha):" in parcel_metadata.get("en", ""):
        total_parcel_area = float(
            parcel_metadata["en"].split("TOTAL ELIGIBLE SURFACE (ha):")[-1]
        )
...
def extract_json_from_reply(raw_text: str, cadastral_ref: str):
    ...
    json_df = pd.json_normalize(data, sep=".", errors="ignore")
    return json_df


def run_vlm_benchmark(
    use_vlm_only: bool = False, lang: str = LANG, use_paper_data: bool = USE_PAPER_DATA
):
    ...
    try:
        ...
        cadastral_ref_list = (
            ...
            if use_paper_data
                ...
                "26002A001000010000EQ",
                "14048A001001990000RM",
                "45054A067000090000QA",
                "43157A024000010000KE",
                "34039A005000020000YQ",
                "27020A319000010000QL",
                "43022A037000430000JO",
                "50074A045000370000KA",
                "50074A014000730000KS",
                "25015A501101860000RF",
                "25142A002000430000BP",
                "22121A007001610000UD",
                "22145A011000110000PI",
                "22061A018000530000GG",
                "26002A004009350000EI",
                "41079A057000020000JR",
                "41012A018000030000TX",
                "23086A02500051FA",
                "23060A065002370000EX",
                "29055A040000040000HL",
                "41062A012001000000UQ",
                "41062A012000960000UB",
                "",
            ]
        )  # TODO
        ...
        for cadastral_ref in cadastral_ref_list:
            ...
            # Parse LLM reply
            json_df = (
                extract_json_from_reply(raw_text.text.strip(), cadastral_ref)
                if not json_data
                else json_data
            )
            if json_data:
                ...
                predicted_base_aid = json_df.get(
                    "Final_Results.Total_Aid_without_Pluriannuality_EUR", [None]
                )[0]
                predicted_plur_aid = (
                    float(
                        json_df.get(
                            "Final_Results.Total_Aid_with_Pluriannuality_EUR", [None]
                        )[0]
                    ),
                )
