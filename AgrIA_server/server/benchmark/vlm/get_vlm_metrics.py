...
def init():
    ...
    input_col_names = ['cadastral_ref', 'image_date',
                       'parcel_desc', 'sr_image_filepath']
    out_col_names = ['cadastral_ref', 'parcel_area', 'land_uses_amount', 'applicable_ecoschemes', 'predicted_ecoschemes', 'applicable_base_aid', 'predicted_base_aid',
                     'applicable_plur_aid', 'predicted_plur_aid', 'ecoschemes_F1', 'base_aid_diff', 'plur_aid_diff', 'base_aid_MAE', 'plur_aid_MAE', 'plur_aid_MAPE', 'base_aid_MAPE', 'exec_time']
...
def setup_ndates(n_dates: int, to_date: datetime = datetime.today(), delta_days: int = 4 * 365):
    ...
    dates = n_random_dates_between(from_date.strftime(
        "%Y-%m-%d"), to_date.strftime("%Y-%m-%d"), n_dates)
...
def get_parcel_data_and_description(cadastral_ref: str, image_date: str, lang: str = LANG):
    ...
    if isinstance(parcel_metadata, dict) and 'TOTAL ELIGIBLE SURFACE (ha):' in parcel_metadata.get('en', ''):
        total_parcel_area = float(parcel_metadata['en'].split(
            "TOTAL ELIGIBLE SURFACE (ha):")[-1])
...
def get_parcel_image(cadastral_ref, geometry, image_date):
    ...
    sr_image_filepath = os.path.join(
        TEMP_DIR, download_sen2sr_parcel_image(geometry, image_date))
    ...
    # Rename file
    filepath_no_ext, ext = os.path.splitext(
        os.path.basename(sr_image_filepath))
    new_image_filepath = BM_SR_IMAGES_DIR / \
        (filepath_no_ext + f"_{cadastral_ref}{ext}")
...
def extract_json_from_reply(raw_text: str, cadastral_ref: str):
    ...
    cleaned_text = "\n".join(
        lines[start_idx:end_idx]).replace("```", "").strip()
...
def run_vlm_benchmark(use_vlm_only: bool = False, lang: str = LANG, use_paper_data: bool = USE_PAPER_DATA):
    ...
    try:
        ...
        cadastral_ref_list = CADASTRAL_REF_LIST_PAPER if use_paper_data else ["26002A001000010000EQ", "14048A001001990000RM", "45054A067000090000QA", "43157A024000010000KE", "34039A005000020000YQ", "27020A319000010000QL", "43022A037000430000JO", "50074A045000370000KA", "50074A014000730000KS", "25015A501101860000RF",
                                                                              "25142A002000430000BP", "22121A007001610000UD", "22145A011000110000PI", "22061A018000530000GG", "26002A004009350000EI", "41079A057000020000JR", "41012A018000030000TX", "23086A02500051FA", "23060A065002370000EX", "29055A040000040000HL", "41062A012001000000UQ", "41062A012000960000UB", ""]  # TODO
        dates = DATES_PAPER if use_paper_data else setup_ndates(
            len(cadastral_ref_list))

        for cadastral_ref in cadastral_ref_list:
            if len(cadastral_ref) != 20:
                continue
            logger.debug(
                f"Waiting {SLEEP_SECONDS} seconds until next parcel report generation...")
            ...
            image_date = dates[i]
            geometry, parcel_desc = get_parcel_data_and_description(
                cadastral_ref, image_date, lang)
            image_filepath = get_parcel_image(
                cadastral_ref, geometry, image_date)
            ...
            raw_text, json_data = get_llm_full_desc(
                image_filepath, parcel_desc, lang)
            # Save full desc to file
            full_desc_filepath = BM_LLM_DIR / \
                f"{cadastral_ref}_full_desc_{lang}.md"
            if json_data:
            ...
            exec_time = str(
                timedelta(seconds=(datetime.now() - init_time).total_seconds()))
            # Parse LLM reply
            json_df = extract_json_from_reply(
                raw_text.text.strip(), cadastral_ref) if not json_data else json_data
            if json_data:
                es_list = sorted({
                    item["Ecoscheme_ID"]
                ...
                predicted_base_aid = json_df.get('Final_Results').get(
                    'Total_Aid_without_Pluriannuality_EUR', [None])
                predicted_plur_aid = json_df.get('Final_Results').get(
                    'Total_Aid_with_Pluriannuality_EUR', [None]),
            else:
                es_list = sorted([
                    item["Ecoscheme_ID"]
                ...
                predicted_base_aid = json_df.get(
                    'Final_Results.Total_Aid_without_Pluriannuality_EUR', [None])[0]
                predicted_plur_aid = float(json_df.get(
                    'Final_Results.Total_Aid_with_Pluriannuality_EUR', [None])[0]),
            ...
            logger.debug(
                f"Time taken for parcel processing {time_taken_formatted}")
        ...
        logger.debug(
            f"Input & output dataframes saved to:\n{input_filepath}\n{out_filepath}")
...
def demo():
    try:
        ...
        logger.exception(
            f"Error during VLM metrics collection:\nEXCEPTION:\t{e}")
    finally:
        logger.info(f"TIMES:\n{dict(times)}")
