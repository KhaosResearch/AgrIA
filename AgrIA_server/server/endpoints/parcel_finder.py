...
def find_parcel():
    ...
    try:
        ...
        parcel_geometry = None if request.form.get(
            'parcelGeometry') == 'None' else request.form.get('parcelGeometry')
        parcel_metadata = request.form.get('parcelMetadata')
        coordinates = None if request.form.get('coordinates') is None else list(
            map(float, request.form.get('coordinates').split(',')))
