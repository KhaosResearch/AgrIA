...
def get_ecoscheme_rules_data(rules_data_list) -> dict:
    ...
    for rule in rules_data_list:
        ...
        threshold = (
            Decimal(threshold_ha)
            if threshold_ha.replace(".", "", 1).isdigit()
            else None
        )
...
def get_base_rate_details(
    rates: dict, threshold: Decimal, keys: list = ["Peninsular", "Insular"]
) -> dict:
    ...
    for key in keys:
        ...
        if key_rates is None:
            continue  # Skip if rate type is missing
        if isinstance(key_rates, dict):
            # Tiered rates (Tier_1, Tier_2, Threshold_ha)
            tier1 = (
                0
                if "/" in str(key_rates["Tier_1"])
                else Decimal(str(key_rates["Tier_1"]))
            )
            tier2 = (
                0
                if "/" in str(key_rates["Tier_2"])
                else Decimal(str(key_rates["Tier_2"]))
            )
            base_rate_details[key] = {
                "Tier_1": tier1,
                "Tier_2": tier2,
            ...
            flat_rate_value = (
                key_rates if isinstance(key_rates, str) else Decimal(str(key_rates))
            )
