...
def percentile_stretch(arr: np.ndarray, p_low=2.0, p_high=98.0) -> np.ndarray:
    ...
    if arr.ndim == 3:
        ...
        for i in range(arr.shape[-1]):
            ...
            out[..., i] = np.clip((band - vmin) / (vmax - vmin) * 255.0, 0, 255).astype(
                np.uint8
            )
        return out
    else:
...
def stack_bgrn(
    b02: BandData, b03: BandData, b04: BandData, b08: BandData
) -> np.ndarray:
    h, w = b02.arr.shape
...
def make_grid(images, ncols=2, pad=4) -> np.ndarray:
    ...
    grid = (
        np.ones(((h + pad) * rows + pad, (w + pad) * ncols + pad, 3), dtype=np.uint8)
        * 255
    )
