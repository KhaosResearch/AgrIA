...
def percentile_stretch(arr: np.ndarray, p_low=2.0, p_high=98.0) -> np.ndarray:
    ...
    if arr.ndim == 3:
        ...
        for i in range(arr.shape[-1]):
            ...
            out[..., i] = np.clip(
                (band - vmin) / (vmax - vmin) * 255.0, 0, 255).astype(np.uint8)
        return out
    else:
...
def stack_bgrn(b02: BandData, b03: BandData, b04: BandData, b08: BandData) -> np.ndarray:
    ...
    out[..., 0], out[..., 1], out[..., 2], out[...,
                                               3] = b04.arr, b03.arr, b02.arr, b08.arr
    return out


def to_torch_4ch(img_bgrn_u16: np.ndarray, device: torch.device) -> torch.Tensor:
    ten = torch.from_numpy(img_bgrn_u16.astype(
...
def make_grid(images, ncols=2, pad=4) -> np.ndarray:
    ...
    norm = [cv2.copyMakeBorder(im, 0, h - im.shape[0], 0, w - im.shape[1],
                               cv2.BORDER_CONSTANT, value=(255, 255, 255)) for im in images]
    n = len(norm)
    rows = math.ceil(n / ncols)
    grid = np.ones(((h+pad)*rows+pad, (w+pad)*ncols+pad, 3),
                   dtype=np.uint8) * 255
