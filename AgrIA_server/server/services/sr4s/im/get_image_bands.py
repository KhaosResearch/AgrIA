...
def download_sentinel_image(lat, lon, size, filename, evalscript):
    ...
    date = datetime(
        year=int(year), month=int(month), day=int(request_date.get().split("-")[-1])
    )
    ...
    logger.debug(f"Sentinel band image saved to {filepath}")


def get_cloudless_image(
    evalscript,
    bbox,
    width,
    height,
    ...
    while attempt < max_tries:
        ...
        try:
            ...
            logger.debug(f"✅ Found valid imagery on attempt {attempt + 1}")
            return img
...
def download_image_bands(lat, lon, size, filename=None, bands=["B02", "B03", "B04"]):
    ...
    for band in bands:
        file_path = (
            filename + f"-{band}_{RESOLUTION}m.tif"
            if filename is not None
            else f"{str(lat)[:8]}_{str(lon)[:8]}-{band}_{RESOLUTION}m.tif"
        )
        logger.debug(f"Fetching images for coordinates: {lat}, {lon}")
