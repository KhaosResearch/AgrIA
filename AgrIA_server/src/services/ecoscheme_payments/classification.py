...
def get_exclusivity_land_uses(
    eligible_schemes_by_land_use, non_eligible_uses, parsed_data, lang="EN"
) -> dict:
    ...
    for land_use_code, data in parsed_data.items():
        ...
        irrigation = float(
            data.get("irrigation_coef", 0.0)[:-1]
        )  # Input format '00.00%'
        slope = (
            float(data.get("slope_coef")[:-1])
            if len(data.get("slope_coef")) > 0
            else 0.0
        )  # Input format '00.00%'

        # Skip non-eligible uses
        if (
            land_use_code in non_eligible_uses
            or land_use_code not in eligible_schemes_by_land_use
        ):
        ...
        for rate_type in ["Peninsular", "Insular"]:
            for scheme in eligible_schemes_by_land_use[land_use_code]:
                ...
                if not is_valid_rate_for_coefficients(
                    scheme_id, scheme_subtype, slope, irrigation
                ):
                    continue
                ...
                if "Flat" in rate_details:
                    current_rate = (
                        Decimal(rate_details["Flat"])
                        if rate_details["Flat"] != "N/A"
                        else Decimal("0")
                    )
                else:
                ...
                if payment_per_ha_total > best_payment_per_ha:
                    ...
                    best_scheme_assignment["payment_per_ha_total"] = (
                        payment_per_ha_total
                    )
...
def get_ecoschemes_rates_and_totals(parsed_data, land_use_assignments) -> dict:
    ...
    for land_use_code, assignment in land_use_assignments.items():
        ...
        scheme_key = (
            f"{scheme_id}_{assignment['subtype']}"
            if scheme_id != "N/A"
            else "Non-Eligible"
        )
        ...
        if scheme_key not in final_scheme_results:
            if scheme_key == "Non-Eligible":
                ...
                final_scheme_results[scheme_key] = {
                    ...
                    "pluriannuality_applicable": assignment[
                        "pluriannuality_applicable"
                    ],
                    "Total_Base_Payment_Peninsular": Decimal("0.0"),
                    "Total_Base_Payment_Insular": Decimal("0.0"),
                }
