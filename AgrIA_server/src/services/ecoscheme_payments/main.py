...
def calculate_ecoscheme_payment(
    ...
    for match in land_use_blocks:
        ...
        parsed_data[land_use_code] = {
            "area": area,
            "irrigation_coef": irrigation_coef,
            ...
            estimated_payments.append(
                {
                    ...
                    "Total_Area_ha": float(
                        area.quantize(ROUNDING_AREA, rounding=ROUND_HALF_UP)
                    ),
    ...
    final_results = {
        ...
        "Total_Aid_without_Pluriannuality_EUR": float(
            total_aid_no_pluriannuality.quantize(
                ROUNDING_PAYMENT, rounding=ROUND_HALF_UP
            )
        ),
        "Total_Aid_with_Pluriannuality_EUR": float(
            total_aid_with_pluriannuality.quantize(
                ROUNDING_PAYMENT, rounding=ROUND_HALF_UP
            )
        ),
    ...
    output_dict = {
        ...
        "Total_Parcel_Area_ha": float(
            total_parcel_area.quantize(ROUNDING_AREA, rounding=ROUND_HALF_UP)
        ),
        "Calculation_Context": {
