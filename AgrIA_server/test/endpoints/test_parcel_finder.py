...
def find_parcel_mocks(monkeypatch):
    ...
    monkeypatch.setattr(
        parcel_finder, "check_cadastral_data", mock_check_cadastral_data
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("40.400409", "-3.631434", 200, True, id="Success_CoordInZone"),
        pytest.param("-40.400409", "3.631434", 200, False, id="Success_CoordNotInZone"),
        pytest.param(
            "abc", "1.0", 400, "Invalid or missing coordinates", id="Failure_BadLat"
        ),
        pytest.param(
            "1.0", "xyz", 400, "Invalid or missing coordinates", id="Failure_BadLng"
        ),
        pytest.param(
            None, None, 400, "Invalid or missing coordinates", id="Failure_NoCoords"
        ),
    ],
)
