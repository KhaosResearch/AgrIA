...
def test_main_calculation_output(sample_input_data_str, mocker):
    ...
    assert output["Final_Results"]["Total_Aid_without_Pluriannuality_EUR"] == 4100.07
    assert output["Final_Results"]["Total_Aid_with_Pluriannuality_EUR"] == 4905.92

    ecoschemes_total_payments = [
        ("P1", 253.97, 253.97),
        ("P3/P4", 3224.27, 3792.96),
        ("P5 (B)", 61.0, 61.0),
        ("P6/P7", 560.83, 797.99),
        ("N/A", "N/A", "N/A"),
    ]
    ...
    for (
        ...
        total_base_payment,
        total_pluri_payment,
    ) in ecoschemes_total_payments:
        # Check Detailed Payments for all ecoschemes
        payment = [
            p
            for p in output["Estimated_Total_Payment"]
            if p["Ecoscheme_ID"] == ecoscheme_id
        ][0]

        # Peninsular (Used for Summary)
        assert payment["Peninsular"]["Total_Base_Payment_EUR"] == total_base_payment
        assert (
            payment["Peninsular"]["Total_with_Pluriannuality_EUR"]
            == total_pluri_payment
        )
