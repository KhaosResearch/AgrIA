...
def test_exclusivity_rule_assignment(parsed_data_fixture, dummy_rules_data):
    ...
    assignments = get_exclusivity_land_uses(
        eligible, non_eligible, parsed_data_fixture)
...
def test_ecoschemes_rates_and_totals_grouping(parsed_data_fixture, dummy_rules_data):
    ...
    assert non_eligible_res['Total_Area_ha'] == Decimal(
        '6.0193')  # FO + CA + IM + ED + ZU
    assert sorted(non_eligible_res['Land_Uses']) == [
        'CA', 'ED', 'FO', 'IM', 'ZU']
