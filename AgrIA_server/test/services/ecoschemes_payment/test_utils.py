...
def test_valid_rate_for_slope_p6_steep():
    ...
    assert (
        is_valid_rate_for_coefficients("P6/P7", "Steep Slope Woody Crops", 15.0, 0.0)
        is True
    )
    # 15% slope, 'Flat Woody Crops' subtype (should fail)
    assert (
        is_valid_rate_for_coefficients("P6/P7", "Flat Woody Crops", 15.0, 0.0) is False
    )
...
def test_valid_rate_for_slope_p6_flat():
    ...
    assert (
        is_valid_rate_for_coefficients("P6/P7", "Steep Slope Woody Crops", 5.0, 0.0)
        is False
    )
...
def test_valid_rate_for_non_restricted_scheme():
    ...
    assert (
        is_valid_rate_for_coefficients("P1", "Mediterranean Pastures", 0.0, 78.0)
        is True
    )
...
def test_calculate_tiered_tier1_payment():
    ...
    area = Decimal("20.0")
    rate_details = {
        "Tier_1": Decimal("100.0"),
        "Tier_2": Decimal("60.0"),
        "Threshold_ha": Decimal("30.0"),
    }
