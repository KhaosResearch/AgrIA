...
def mock_sen2sr_dependencies(monkeypatch):
    ...
    mlstac_load_mock = MagicMock(return_value=MagicMock(compiled_model=lambda device: MockCompiledModel()))
    monkeypatch.setattr(mlstac, "load", mlstac_load_mock)
    
    # --- Data & Tensor Prep Mocks ---
    monkeypatch.setattr(get_sr_image, "lonlat_to_utm_epsg", MagicMock(return_value=MOCK_CRS))
    monkeypatch.setattr(get_sr_image, "download_sentinel_cubo", MagicMock(return_value=(MockCloudlessData(), MOCK_SAMPLE_DATE)))
    ...
    sen2sr_predict_large_mock = MagicMock(return_value=torch.rand(4, 512, 512)) # 4x upscaled mock
    ...
    reordered_mock = (torch.rand(4, 128, 128).numpy(), torch.rand(4, 512, 512).numpy())
    ...
    monkeypatch.setattr(get_sr_image, "make_pixel_faithful_comparison", MagicMock())
    
    crop_parcel_mock = MagicMock(return_value=MOCK_SR_FILEPATH)
    monkeypatch.setattr(get_sr_image, "crop_parcel_from_sr_tif", crop_parcel_mock)
